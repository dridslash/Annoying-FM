...
public class PlayerController : ExtendedMonoBehaviour
{
	...
	private void Update()
	{
		...
		if (IsGameStarted && !IsGameCompleted && !IsGameOver && !sleeping && !IsSleeping()) //replace true by "game running boolean" and uncomment else statement
		{
	...
	public void WakeUp()
	{
		...
		AudioManager.instance.Stop("Sleeping");
		sleeping = false;
	}
	...
	bool IsSleeping()
	{
		return animator.GetCurrentAnimatorStateInfo(0).IsName("Sleeping");
	}
